...
# Root Directories
GPUS="0" # GPU size for tensor_parallel.
...
IFS=":" read MODEL_PATH MODEL_TEMPLATE_TYPE MODEL_FRAMEWORK TOKENIZER_PATH TOKENIZER_TYPE MODEL_API_KEY GEMINI_API_KEY AZURE_ID AZURE_SECRET AZURE_ENDPOINT <<< "$MODEL_CONFIG"
...
# In CI, use only the first two sequence lengths and first two tasks
if [ "${CI}" = "true" ]; then
    SEQ_LENGTHS=(${SEQ_LENGTHS[@]:0:2})
    TASKS=(${TASKS[@]:0:2})
